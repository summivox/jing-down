...
var Core = {
    ...
    A64: "64",
    init: function() {
        Core.standalone || (Core.standalone = !1),
    ...
    setCookie: function(name, value, expires, path, domain, secure) {
        ...
        domain = "." + Core.DOMAIN,
        expires = Core.getExpDate(365, 0, 0),
...
resStyle.thm = new Array(390, 426, 0),
...
Signup = {
    ...
    init: function() {
        var e = function() {
            Signup.userDetail.invitation = window.location + "",
            Signup.userDetail.invitation == undefined && (Signup.userDetail.invitation = ""),
            Signup.userDetail.invitation.indexOf("?") == -1 ? Signup.userDetail.invitation = "": (Signup.userDetail.invitation = Signup.userDetail.invitation.substring(Signup.userDetail.invitation.indexOf("?") + 1), Signup.userDetail.invitation.indexOf("forgot=") == 0 ? (Signup.resetPwdToken = Signup.userDetail.invitation.replace("forgot=", ""), Signup.userDetail.invitation = "", $.ajax({
        ...
        Core.getCookie("jing.auth") != "" && Core.getCookie("jing.auth") != null ? $.ajax({
            ...
            success: function(t) {
                t.success ? (clearTimeout(Signup.tpsTmo), Signup.setUserDetail(t.result), Signup.finishClick()) : e()
            }
        }) : e(),
        $(".bdgCtn").css({
    ...
    authCallback: function(e) {
        ...
        } else e.result.association ? (Signup.setUserDetail(e.result), Signup.userDetail.newbie == 2 ? Signup.bdgAnimate() : Signup.finishClick()) : Signup.snsAnimate(e);
    ...
    bindBtnClick: function() {
        ...
        $.ajax({
            ...
            success: function(t) {
                t.success ? (Signup.setUserDetail(t.result), Signup.userDetail.newbie == 1 ? $.ajax({
                    url: Core.API_VER + "/account/update_profile",
                    data: {
                        uid: Signup.userDetail.id,
                        nick: e,
                    ...
                    success: function(e) {
                        e.success ? Signup.signupCallback(e) : ($("#bindBtn").removeClass("selected"), alert(e.codemsg))
                    }
                }) : Signup.signupCallback(t)) : ($("#bindBtn").removeClass("selected"), Signup.tpsShow($("#snsPwd"), t.codemsg))
            }
        });
    ...
    setUserDetail: function(e) {
        Signup.userDetail.menu = e.m,
        Signup.userDetail.pld = e.pld,
        Signup.userDetail.k = e.k;
        if (e.avbF != null) {
            var t = e.avbF.split(",");
            for (var n = 0; n < t.length; ++n) Signup.userDetail.avbF[t[n]] = !0
        }
...
Overlay = {
    ...
    getMaxCount: function() {
        ...
        Overlay.id == "hate" || Overlay.id == "love" || Overlay.id == "invtApp" || Overlay.id == "chtApp" ? Overlay.maxRowCount = 1 : Overlay.id == "tkrs" ? Overlay.maxRowCount = Overlay.maxRowCount > 2 ? 2 : Overlay.maxRowCount: Overlay.id == "olFrd" || Overlay.id == "inHs" || Overlay.id == "rbsFrd" || Overlay.id == "qq" || Overlay.id == "rr" || Overlay.id == "wb" || Overlay.id == "db" ? Overlay.maxRowCount = Overlay.maxRowCount > 3 ? 3 : Overlay.maxRowCount: Overlay.id == "invtApp" ? Overlay.maxColCount = 1 : bdgId[Overlay.id] != undefined && (Overlay.maxRowCount = Overlay.maxRowCount > 4 ? 4 : Overlay.maxRowCount)
    },
    switchAnimate: function(e) {
    ...
    more: function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.result.total <= response.result.st + response.result.items.length && $("#menuMore").remove(),
                Overlay.result = response.result,
                Overlay.moreAnimate(),
...
Player = {
    ...
    setPlay: function(e, t, n) {
        ...
        Player.seconds = e,
        Player.currentTime >= 1 && $.ajax({
            url: "/official/stat?Heard|" + Signup.userDetail.id + "|" + Player.music[Player.pos].tid + "|" + Player.currentTime
        });
        var r = $.id2mediaUrl(t, "audio");
        r += "?start=" + e,
...
Apps = {
    ...
    thmHtml: function() {
        var e = new Object;
        e.rdpnk = new Array("橙色年华", 6),
        e.dflt = new Array("淡淡时光", 0),
        e.blkstn = new Array("黑色迷墙", 2),
        e.lthbg = new Array("复古质感", 2),
        e.ctwd = new Array("雕刻时光", 4),
        e.rlwd = new Array("一叶知秋", 3);
        var t = Overlay.result.items[0].v,
        n = "";
        for (var r in e) {
            var i = "";
            t == r && (i = "selected");
            var s = "";
            Core.isNewIcon("t", r) && (s = '<em class="newApp" style="left:auto; top:7px; right:20px;"></em>', Core.ulNewIcon("t", r)),
            n += '<div class="rctCtn thm">' + s + '<div class="thmIcon ' + r + '"></div>' + '<p class="ctn">' + e[r][0] + "</p>" + '<a id="' + r + '" href="#" class="chc ' + i + ' swtchEvent"></a>' + "</div>"
        }
        $("#appsCttCtn").html(n),
        $(".swtchEvent").click(function() {
            var t = "";
            if (!$(this).isClass("selected")) {
                t = $(this).attr("id");
                for (var n in e) $("#" + n).removeClass("selected");
                $(this).addClass("selected"),
                Gns.nowGns('你正在更换 <a class="trg" href="#">' + e[t][0] + "</a> 主题。"),
                $.ajax({
                    url: Core.API_VER + "/setting/post_settings",
                    data: {
                        uid: Signup.userDetail.id,
                ...
                var r = function() {
                    for (var n in e) $("html").removeClass(n + "Thm");
                    $("html").addClass(t + "Thm");
                    var r = "";
                    t == "lthbg" || t == "ctwd" ? r = "-17px": r = "-14px",
                    $("#mainMenuCtn").animate({
                        bottom: r
                    },
                    300),
                    Signup.userDetail.sts.thm = t
                };
                r()
            }
        })
